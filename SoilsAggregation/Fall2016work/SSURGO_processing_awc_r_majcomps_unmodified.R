...
#paw_check result was using paw_est = awc_r for each horizon to calculate paw estimate and AD calculated by % AD * awc_r per horizon
#allowable_depletion_tension function is calculated using decision tree 1 to define FC.  However, this data is not aggregated.
#allowable_depletion function is modified so that AD = AD% x awc_r for each horizon. Results from this function are aggregated to component and map unit levels.
library(plyr)
library(dplyr)
library(nnet)
library(extrafont)
...
results <- file.path(mainDir, 'soils_data/results/paw_check_majcomps_unmodified')
fc_def <- 'awc_r_majcomps'
figure_label <- 'SSURGO defined FC, majcomps only'
...
#insert vg equation here and adjust with fragvol_r; still need to determine how to use salinity data
  ...
  ssurgo_horizon$paw_est <- ssurgo_horizon$awc_r
  #allowable_depletion_tension('theta_15bars', ssurgo_horizon) #make this equal to awc_r for 'paw_est' method
...
#second, for alfalfa, sum up awc by cokey for those without paralithic or lithic contacts but without modification of the soil profile.
  ...
  colnames(comps_no_Cr_R) <- 'AD_alfalfa_cmH2O' #unmodified version
  ...
  comps_no_Cr_R$aws0150ck_cmH2O <- tapply(ssurgo_horizon_no_Cr_R$aws0150ck_cmH2O, ssurgo_horizon_no_Cr_R$cokey, sum_modified) #unmodified version
  comps_no_Cr_R$aws0150ck_soilthickness <- tapply(ssurgo_horizon_no_Cr_R$aws050ck_soilthickness, ssurgo_horizon_no_Cr_R$cokey, sum_modified) #unmodified version
...
#third, for walnuts, almonds, and grapes, sum up awc by cokey for those without paralithic or lithic contacts by adding missing AD data for the portion of the profile of the rooting zone without it, assuming profile wtd avgs for the missing data.  AD_'crop'_soilthickness reflects cm of rooting zone with data as a QC check. also do this for the 0-150 cm aws check.
  ...
  comps_no_Cr_R$AD_grapes_soilthickness <- tapply(ssurgo_horizon_no_Cr_R$AD_grapes_soilthickness, ssurgo_horizon_no_Cr_R$cokey, sum_modified) #unmodified version
  comps_no_Cr_R$AD_almonds_soilthickness <- tapply(ssurgo_horizon_no_Cr_R$AD_almonds_soilthickness, ssurgo_horizon_no_Cr_R$cokey, sum_modified) #unmodified version
  comps_no_Cr_R$AD_walnuts_soilthickness <- tapply(ssurgo_horizon_no_Cr_R$AD_walnuts_soilthickness, ssurgo_horizon_no_Cr_R$cokey, sum_modified) #unmodified version
  comps_no_Cr_R$AD_grapes_cmH2O <- tapply(ssurgo_horizon_no_Cr_R$AD_grapes_cmH2O_unmodified, ssurgo_horizon_no_Cr_R$cokey, sum_modified) #unmodified version
  comps_no_Cr_R$AD_almonds_cmH2O <- tapply(ssurgo_horizon_no_Cr_R$AD_almonds_cmH2O_unmodified, ssurgo_horizon_no_Cr_R$cokey, sum_modified) #unmodified version
  comps_no_Cr_R$AD_walnuts_cmH2O <- tapply(ssurgo_horizon_no_Cr_R$AD_walnuts_cmH2O_unmodified, ssurgo_horizon_no_Cr_R$cokey, sum_modified) #unmodified version
...
#merge and rbind data.frames to fill in data for minor components based off of component name averages
  ssurgo_comp_no_rock <- merge(ssurgo_comp_no_rock, comps_no_Cr_R, by='cokey', all = TRUE)
...
#temp edit to read in mu_aggregated_data_CA_ag.csv file
  results <- file.path(mainDir, 'soils_data/results/paw_check_majcomps_unmodified')
  fc_def <- 'awc_r_majcomps'
  figure_label <- 'SSURGO defined FC, majcomps only'
  setwd(results)
  ...
  plot(final_result$aws0150ck_cmH2O_wtdavg, final_result$aws0150wta, type='p', xlab=expression('SSURGO 0-150 cm PAW (cm H'[2]*'O), data aggregation test'), ylab=expression('SSURGO muggatt 0-150 cm PAW (cm H'[2]*'O)'), cex.lab=1.3, cex.axis=1.3, col='blue1')
