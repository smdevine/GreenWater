...
#modified allowable depletion function to use only internal SSURGO data for water content at 1/3 and 15 bars used to calculate PAW; not 1/3 and 15 bar predictions from Rosetta
#negative PAW values converted to NA under this method
library(plyr)
library(dplyr)
library(nnet)
library(extrafont)
...
results <- file.path(mainDir, 'soils_data/results/SSURGO_FC_0.33bar')
fc_def <- 'wthirdbar_r'
figure_label <- 'SSURGO FC -0.3bar'
...
#   # ssurgo_horizon$field_cap <- fc_correction_vector
  ssurgo_horizon$wthirdbar_r <- ssurgo_horizon$wthirdbar_r/100
  ...
  ssurgo_horizon$wfifteenbar_r <- ssurgo_horizon$wfifteenbar_r/100
  ...
  allowable_depletion <- function(df, percent) { #see bottom of script for another way to write this kind of a fucntion
    for (i in 1:nrow(df)) {
      ...
      if (is.na(df[['field_cap']][i])) {
        y <- c(y, NA)
        next
      }
      if (is.na(df[['wfifteenbar_r']][i])) {
        y <- c(y, NA)
        next
      }
      # if (df[['Ros_model']][i]==5) { #if theta @ 0.33 and 15 bars were used in the Rosetta model prediction, don't need to correct for fragment volume
      if ((df[['field_cap']][i] - df[['wfifteenbar_r']][i]) < 0) { #if less than 0, mark as NA
        y <- c(y, NA)
        next
      }
      y <- c(y, (df[['field_cap']][i] - df[['wfifteenbar_r']][i])*percent/100) #otherwise record the calc
      next
      # }
      # if (df[['Ros_model']][i]==2) {
      #   y <- c(y, (df[['field_cap']][i] - df[['wfifteenbar_r']][i])*(percent/100)*(1 - df[['fragvol_r_sum']][i]/100)) #if only texture was used in the Rosetta model prediction, then need to correct for rock fragments
      #   next
      # } #not necessary, because not using Ros_model data for this script
    }
    invisible(y)
  }
  ...
  ssurgo_horizon$paw_est <- ssurgo_horizon$field_cap - ssurgo_horizon$wfifteenbar_r #make this equal to awc_r for 'paw_est' method
  ssurgo_horizon$paw_est[which(ssurgo_horizon$paw_est < 0)] <- NA #convert negative calcs to 0 due to bad data
...
#removed read in step here since this was a new procedure
  final_result_qc <- final_result[which(final_result$aws_ppct_tot > 75), ] #represents 7,011,392 ha
  final_result_qc <- final_result_qc[which(final_result_qc$aws0150ck_cmH2O_wtdavg >= 0), ] #get rid of negative values (apparent errors where theta 15 > theta 0.33 in SSURGO)
