...
plot(california_aea)
plot(crop_tile1, add=T, legend=F)
plot(crop_tile2, add=T, legend=F)
plot(crop_tile3, add=T, legend=F)
plot(crop_tile4, add=T, legend=F)
plot(extent1, add=T)
plot(extent2, add=T)
plot(extent3, add=T)
plot(extent4, add=T)
...
results_CA_TA <- spTransform(results_df, crs(california)) #California Teale Albers coordinates for getting SpatialCimis cell numbers
results_df_CA_TA <- as.data.frame(results_CA_TA)
head(results_df_CA_TA)
colnames(results_df_CA_TA)[3:4] <- c('longitude_CA_TA', 'latitude_CA_TA')
setwd('C:/Users/smdevine/Desktop/Allowable_Depletion/results/data.frames')
...
setwd(file.path(results, 'data.frames'))
write.csv(mukeys_cropcodes_longlatNAD83, 'mukeys_cropcodes_longlatNAD83.csv')
...
setwd(file.path(spatialCIMIS, '2004'))
spatialCIMIS <- raster('ETo_20040101.tif')
CIMIScellnumbers <- as.integer(cellFromXY(object=spatialCIMIS, xy=mukeys_cropcodes_CA_TA))
summary(CIMIScellnumbers)
CIMIScellunique <- unique(CIMIScellnumbers) #17,341 CIMIS cells need sampling
setwd(file.path(results, 'model_scaffold'))
...
setwd(file.path(PRISMDir, 'PRISM_ppt_stable_4kmD2_20031001_bil'))
...
PRISMcellunique <- unique(PRISMcellnumbers) #5338 PRISM cells need sampling
...
setwd(file.path(results, 'model_scaffold'))
write.csv(model_scaffold, 'model_scaffold_codes_nocoords_6.9.17.csv', row.names = F) #this destroys the model code numbers when read back in, so they were deleted to avoid future confusion. they can simply be re-created by paste0(results_df$mukey, results_df$crop_code, results_df$PRISMcellnumber, results_df$CIMIScellnumber) in this order
model_scaffold <- read.csv('model_scaffold_codes6.8.17.csv')
head(model_scaffold)
