...
#modified KeiCalc and KepCalc on 9/11/17 to correct for overestimation of evaporation from sandy soils with very low TEW (i.e. <12 mm)
  ...
  #14.  Deep percolation result for the growing season period [DONE]
  #15.  Do separate water balance tracking for irrigation and green water to get a more temporally accruate summation of green water utilization; think about implications of mixing surface water with deep root zone water [IN-PROGRESS]
  #16.  Add summation of Ep over growing season and winter (how will adding new variable columns affect aggregation and synthesis of results?)
  #17.  Add water balance check result to each unique model code (add new data column to results data.frame) [IN-PROGRESS]
...
resultsDir <- 'C:/Users/smdevine/Desktop/Allowable_Depletion/results/Jan2018'
rounding_digits <- 3
...
FAO56DualCropCalc <- function(cropname, cropcode, AD.percentage, root_depth, irr.type, crop.parameters.df, model.scaffold, U2.df, P.df, ETo.df, RHmin.df, results_file, row_start, RDI.min, alfalfa.zone, grape.zone) {
  ...
  #addtional functions to track ET of P explicitly

  #set 1 functions, use initial final estimate of soil stored P to guide fraction of ET and DPr as P not irr water
  SpInitialCalc <- function(Sp.end, ETc.act, Dr.end, PAW, P, DPr, i) {#depth of preciptiation water held in root zone
    Sp.end[i - 1] + P[i] - (Sp.end[i - 1] / (PAW - Dr.end[i - 1])) * (DPr[i] + ETc.act[i]) #not to be used for i=1
  }
  SpEndCalc <- function(Sp.end, ETc.P, Dr.end, PAW, P, DPr, i) {#depth of preciptiation water held in root zone
    max(min(Sp.end[i-1] - ETc.P[i] + P[i] - DPr.P[i], PAW), 0) #not to be used for i=1
  }
  ETc.PCalc <- function(Sp.initial, PAW, Dr.end, ETc.act, i) {
    (Sp.initial[i] / (PAW - Dr.end[i])) * ETc.act[i] 
  }
  DPr.PCalc <- function(Sp, PAW, Dr.end, DPr, i) {
    (Sp.initial[i] / (PAW - Dr.end[i])) * DPr[i]
  }
  T.PCalc <- function(Kcb, Ks, Sp.end, PAW, Dr.end, ETo) {
    (Ks[i] * Kcb[i] * (Sp.initial[i] / (PAW - Dr.end[i]))) * ETo[i] 
  }
  #set 2 functions, use i-1 soil storage of P to plant available water to determine fraction of ET and DPr as P not Irr water
  SpEndCalc <- function(Sp.end, ETc.P, Dr.end, PAW, P, DPr, i) {#depth of preciptiation water held in root zone
    max(min(Sp.end[i-1] - ETc.P[i] + P[i] - DPr.P[i], PAW), 0) #not to be used for i=1
  }
  ETc.PCalc <- function(Sp.end, PAW, Dr.end, ETc.act, i) {
    (Sp.end[i-1] / (PAW - Dr.end[i-1])) * ETc.act[i] 
  }
  DPr.PCalc <- function(Sp, PAW, Dr.end, DPr, i) {
    (Sp.end[i-1] / (PAW - Dr.end[i-1])) * DPr[i]
  }
  ...
  WaterBalanceCalc <- function(df) { #concept is to run by year on a data.frame trimmed to Jdev-Jharv each year
    ...
    if (df$doys.model[1] > Jdev | df$doys.model[nrow(df)] < Jharv) { #if there is not a complete growing season, don't report any data
      ...
      if (length(last.irr.index)==0) { #but no final irrigation occurred
        data.frame(RAW.end.season = max(AD - df$Dr.end[jharv_index], 0), PAW.end.season = max(PAW - df$Dr.end[jharv_index], 0), Dr.end.season = df$Dr.end[jharv_index], P.end.season = NA, Irr.end.storage = NA, GW.ET.growing = sum(df$ETc.P[jdev_index:jharv_index]), Irr.app.total = sum(df$Ir), Irr.app.last = NA, ET.growing = sum(df$ETc.act[jdev_index:jharv_index]), E.growing = sum(df$Ei[jdev_index:jharv_index], df$Ep[jdev_index:jharv_index]), T.growing = sum(df$ETc.act[jdev_index:jharv_index] - df$Ei[jdev_index:jharv_index] - df$Ep[jdev_index:jharv_index]), H2O.stress=sum(df$ETc.ns - df$ETc.act))
      } else {
        irr_storage <- max(AD - df$Dr.end[jharv_index] - sum(df$P[last.irr.index:jharv_index]), 0)
      data.frame(RAW.end.season = max(AD - df$Dr.end[jharv_index], 0), PAW.end.season = max(PAW - df$Dr.end[jharv_index], 0), Dr.end.season = df$Dr.end[jharv_index], P.end.season = sum(df$P[last.irr.index:jharv_index]), Irr.end.storage = irr_storage, GW.ET.growing = sum(df$ETc.P[jdev_index:jharv_index]), Irr.app.total = sum(df$Ir), Irr.app.last = df$Ir[last.irr.index], ET.growing = sum(df$ETc.act[jdev_index:jharv_index]), E.growing = sum(df$Ei[jdev_index:jharv_index], df$Ep[jdev_index:jharv_index]), T.growing = sum(df$ETc.act[jdev_index:jharv_index] - df$Ei[jdev_index:jharv_index] - df$Ep[jdev_index:jharv_index]), H2O.stress=sum(df$ETc.ns - df$ETc.act))
      }
    }
  }
  ...
  #modified definitions of green water to work off of ETc.P
  GreenWaterIrr1Calc <- function(df) { #works on a data.frame split by year
    ...
    if (df$doys.model[1] > Jdev | length(first.irr.index)==0) {
      ...
      data.frame(GW.ET.to.Irr1 = sum(df$ETc.P[jdev_index:first.irr.index]), GW.E.to.Irr1 = sum(df$Ei[jdev_index:first.irr.index] + df$Ep[jdev_index:first.irr.index]), GW.T.to.Irr1 = sum(df$Kcb.adjusted[jdev_index:first.irr.index] * df$Ks[jdev_index:first.irr.index] * df$ETo[jdev_index:first.irr.index]))
    }
  }
  ...
  DeepPercCalc <- function(df) { #assumes Jharv index is after 10/1
    ...
    if (df$dates[1] > as.Date(paste0(as.character(df$years[1]), '-01-01')) | df$dates[nrow(df)] < as.Date(paste0(as.character(df$years[nrow(df)]), '-12-31'))) { #this means they ain't a whole year's data
      if (length(last.irr.index) == 0 & length(first.irr.index) == 0) { #not sufficient coverage to calc anything
        data.frame(ET.annual = NA, E.annual = NA, T.annual = NA, deep.perc.annual = NA, winter.deep.perc = NA, post.Irr1.deep.perc=NA, fall.deep.perc = NA, deep.perc.growing=NA)
      }
      else if (length(jan.1.index) != 0 & length(first.irr.index) != 0 & length(jul.1.index) == 0) { #winter deep perc only
        data.frame(ET.annual = NA, E.annual = NA, T.annual = NA, deep.perc.annual = NA, winter.deep.perc = sum(df$DPr[jan.1.index:first.irr.index]), post.Irr1.deep.perc=NA, fall.deep.perc = NA, deep.perc.growing=NA)
      }
      else if (length(jan.1.index) != 0 & length(jul.1.index) != 0 & length(dec.31.index) == 0) { #winter and post Irr deep perc
        data.frame(ET.annual = NA, E.annual = NA, T.annual = NA, deep.perc.annual = NA, winter.deep.perc = sum(df$DPr[jan.1.index:first.irr.index]), post.Irr1.deep.perc=if(first.irr.index < jul.1.index) {sum(df$DPr[(first.irr.index+1):jul.1.index])}else{NA}, fall.deep.perc = NA, deep.perc.growing=NA)
      }
        ...
        data.frame(ET.annual = NA, E.annual = NA, T.annual = NA, deep.perc.annual = NA, winter.deep.perc = NA, post.Irr1.deep.perc=if(first.irr.index < jul.1.index) {sum(df$DPr[(first.irr.index+1):jul.1.index])}else{NA}, fall.deep.perc = NA, deep.perc.growing=NA)
      }
      else if (length(jan.1.index) == 0 & length(first.irr.index) != 0 & length(dec.31.index) != 0) { #spring and fall deep perc
        data.frame(ET.annual = NA, E.annual = NA, T.annual = NA, deep.perc.annual = NA, winter.deep.perc = NA, post.Irr1.deep.perc=if(first.irr.index < jul.1.index) {sum(df$DPr[(first.irr.index+1):jul.1.index])}else{NA}, fall.deep.perc =  sum(df$DPr[last.irr.index:dec.31.index]), deep.perc.growing=NA)
      } else {#fall perc only
        data.frame(ET.annual = NA, E.annual = NA, T.annual = NA, deep.perc.annual = NA, winter.deep.perc = NA, post.Irr1.deep.perc=NA,  fall.deep.perc =  sum(df$DPr[last.irr.index:(dec.31.index-1)]), deep.perc.growing=NA)
      }
    } else { #entire annual coverage available
      if (length(first.irr.index)==0 & length(last.irr.index)==0) { #but no irrigations
        data.frame(ET.annual = sum(df$ETc.act), E.annual = sum(df$Ei, df$Ep), T.annual = sum(df$ETc.act, -df$Ei, -df$Ep), deep.perc.annual = sum(df$DPr), winter.deep.perc = NA, post.Irr1.deep.perc = NA, fall.deep.perc = NA, deep.perc.growing=sum(df$DPr[jdev_index:jharv_index]))
      } else {
        data.frame(ET.annual = sum(df$ETc.act), E.annual = sum(df$Ei, df$Ep), T.annual = sum(df$ETc.act, -df$Ei, -df$Ep), deep.perc.annual = sum(df$DPr), winter.deep.perc = sum(df$DPr[jan.1.index:first.irr.index]), post.Irr1.deep.perc = if(first.irr.index < jul.1.index) {sum(df$DPr[(first.irr.index+1):jul.1.index])}else{NA}, fall.deep.perc = sum(df$DPr[last.irr.index:dec.31.index]), deep.perc.growing=sum(df$DPr[jdev_index:jharv_index]))
      }
    }
  }
...
#loop through all rows of model scaffold but only do these operations once for each model.scaffold.crop row
  ...
  #identify climatic and soil parameters
    ...
    Sp.initial <- numeric(length = model.length)
    ETc.P <- numeric(length = model.length)
    DPr.P <- numeric(length = model.length)
    Sp.end <- numeric(length = model.length)
  ...
  #and Kr.  Same done for Dep.initial
    ...
    Sp.initial[1] <- P[1]
    ETc.P[1] <- ETc.PCalc(Kc.act, Sp.initial, PAW, Dr.end, ETo)
    DPr.P[1] <- DPr.PCalc(Sp.initial, PAW, Dr.end, DPr)
    Sp.end[1] <- P[1] - ETc.P[1] - DPr.P[1] #initial calc
