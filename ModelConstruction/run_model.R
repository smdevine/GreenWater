...
#re-run test on 10/17 due to Ks function call argument error  that should only affect 30 and 80% AD scenarios
modelgrid <- expand.grid(root.depths = c('1.0m', '2.0m', '3.0m'), AD.percentage = c(30, 50, 80), cropnames = c('walnut.mature', 'almond.mature', 'pistachios', 'grapes.table', 'grapes.wine', 'alfalfa.intermountain', 'alfalfa.CV', 'alfalfa.imperial'))
modelgrid$root.depths <- as.character(modelgrid$root.depths)
modelgrid$cropnames <- as.character(modelgrid$cropnames)
modelgrid$AD.percentage <- as.integer(modelgrid$AD.percentage)
