...
resultsDir <- 'D:/Allowable_Depletion/results/Apr2018sensitivity.analysis'
rounding_digits <- 3
...
FAO56DualCropCalc <- function(cropname, cropcode, AD.percentage, root_depth, irr.type, crop.parameters.df, model.scaffold, U2.df, P.df, ETo.df, RHmin.df, results_file, row_start, RDI.min, alfalfa.zone, grape.zone, stress.assumption, dailyWBsave, scenario.dir, bloom.offset, surface.assumption) {
  ...
  CropParametersDefine <- function(crop.parameters, cropname, bloom.offset) {
    ...
    if (cropname=='alfalfa.intermountain' | cropname=='alfalfa.CV' | cropname=='alfalfa.imperial') {
      ...
      crop.parameters$Jdev <- as.integer(format.Date(bloom.date, '%j')) + bloom.offset
      crop.parameters$Jmid <- crop.parameters$Jdev + crop.parameters$Ldev
      crop.parameters$Jlate <- crop.parameters$Jmid + crop.parameters$Lmid
      crop.parameters$Jharv <- crop.parameters$Jlate + crop.parameters$Llate
      return(crop.parameters)
...
#do.call(rbind, lapply(split(model.result, model.result$years), IrDateCalc))
  ...
  if (!dir.exists(file.path(resultsDir, scenario.dir))) {
    dir.create(file.path(resultsDir, scenario.dir))
  }
  if (!dir.exists(file.path(resultsDir, scenario.dir, cropname.dir))) {
    dir.create(file.path(resultsDir, scenario.dir, cropname.dir))
  }
  if (!dir.exists(file.path(resultsDir, scenario.dir, scenario.name))) {
    dir.create(file.path(resultsDir, scenario.dir, scenario.name))
  }
...
#assumes depletion is zero for root zone except for half of TEW in upper layer
  ...
  #and Kr.  Same done for Dep.initial
    ...
    if (n==1 | n %in% rows.to.sample) {
      ...
      write.csv(model.result, file.path(resultsDir, scenario.dir, scenario.name, paste0(cropname, root_depth, 'AD', as.character(AD.percentage), '_', as.character(model.code), '_', as.character(cokey), '_', Sys.Date(), '.csv')), row.names=FALSE)
    }
    if (n %in% save.times) { #was (n==100 | n %in% save.times)
      write.csv(model.scaffold.results, file.path(resultsDir, scenario.dir, scenario.name, paste0(cropname, root_depth, 'AD', as.character(AD.percentage), '_FAO56results.csv')), row.names=FALSE)
      write.csv(QC.results, file.path(resultsDir, scenario.dir, scenario.name, paste0(cropname, root_depth, 'AD', as.character(AD.percentage), '_model_QC.results.csv')), row.names=FALSE)
    } else {next}
...
#test_function
FAO56DualCropCalc(cropname = 'almond.mature', cropcode = almond_code, AD.percentage = 50, root_depth = '0.5m', irr.type = 'Border', crop.parameters.df = crop.parameters.df, model.scaffold = model.scaffold, U2.df = U2.df, P.df = P.df, ETo.df = ETo.df, RHmin.df = RHmin.df, results_file = 'new', row_start = 1, RDI.min = NA, alfalfa.zone = NA, grape.zone = NA, stress.assumption = 0.5, dailyWBsave = FALSE, scenario.dir = 'Border.irrigation')
###END OF FUNCTION
